...
const { validateCategoryUpdate, validateFoodAndDrinkCategoryUpdate, validateId } = require("../utils/request-validator");
...
async function updateCategory(req, res, next) {
    try {
        ...
        const error = await validateFoodAndDrinkCategoryUpdate(category, uploadedFile?.mimetype);
        if (Object.keys(error).length) {
            throw new HttpException(400, "Bad request.", error);
        }
...
async function deleteCategory(req, res, next){
    try {
        ...
        foodAndDrinkService.deleteFoodAndDrinkByCategory(categoryId);
